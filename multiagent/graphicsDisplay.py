...
# Hole
HOLE_COLOR = formatColor(50, 50, 50)
HOLE_SIZE = 0.6

# Jump
JUMP_COLOR = formatColor(250, 128, 114)
JUMP_SIZE = 0.25
...
class PacmanGraphics:
    ...
    def drawStaticObjects(self, state):
        ...
        if layout.jump:
            self.jump = self.drawJump(layout.jump)
        refresh()
    ...
    def update(self, newState):
        ...
        if newState._jumpEaten != None:
            self.removeJump(newState._jumpEaten, self.jump)
    ...
    def drawHoles(self, holes):
        holeImages = {}
        for hole in holes:
            (screen_x, screen_y) = self.to_screen(hole)
            dot = circle((screen_x, screen_y),
                         HOLE_SIZE * self.gridSize,
                         outlineColor=HOLE_COLOR,
    ...
    def drawJump(self, jump):
        jumpImage = {}
        (screen_x, screen_y) = self.to_screen(jump)
        dot = circle((screen_x, screen_y),
                     JUMP_SIZE * self.gridSize,
                     outlineColor=JUMP_COLOR,
                     fillColor=JUMP_COLOR,
                     width=1)
        jumpImage[jump] = dot
        return jumpImage
    ...
    def removeJump(self, cell, jumpImage):
        x, y = cell
        remove_from_screen(jumpImage[(x, y)])
